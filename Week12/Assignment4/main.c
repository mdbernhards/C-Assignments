...
/// Calculates days between two given dates.
/// @param dateOne The first date.
/// @param dateTwo The second date.
/// @return days between two given dates.
int CalculateDaysBetweenTwoDates(const int birthDateArray[], int currentDateArray[])
{
    int daysBetweenTwoDates = 0;

    for (int i = birthDateArray[2]; i < currentDateArray[0]; i++)
    {
        if (IsLeapYear(i))
        {
            daysBetweenTwoDates += 366;
        }
        else
        {
            daysBetweenTwoDates += 365;
        }
    }

    daysBetweenTwoDates += (GetDayOfTheYear(currentDateArray[2], currentDateArray[1], currentDateArray[0])
        - GetDayOfTheYear(birthDateArray[0], birthDateArray[1], birthDateArray[2]));

    return daysBetweenTwoDates;

    // create a for loop:
        // for each year check if it is a leap year with IsLeapYear
        // if it is add 366 to daysBetweenTwoDates, if not then 365
    // end loop

    // add days of the last year to daysBetweenTwoDates with GetDayOfTheYear(currentDateArray[0], currentDateArray[1], currentDateArray[2]);
    // return daysBetweenTwoDates
}

int main(void)
{
    int birthDateArray[ARRAY_SIZE], daysBetweenTwoDates, currentDateArray[ARRAY_SIZE];

    get_current_date(currentDateArray);
    currentDateArray[2] = 1;
    currentDateArray[1] = 1;
    currentDateArray[0] = 2020;
    currentDateArray[0] = 2021;

    printf("Current date: %d-%d-%d \n", currentDateArray[2], currentDateArray[1], currentDateArray[0]);

    GetValidDate(birthDateArray);
    daysBetweenTwoDates = CalculateDaysBetweenTwoDates(birthDateArray, currentDateArray);
    ...
    {
        printf("Error: You have not been born yet!");
    }


    // declare int birthDateArray[ARRAY_SIZE]; for date given by user
    // declare int daysBetweenTwoDates, int currentDateArray[ARRAY_SIZE];

    // get current date with get_current_date[currentDateArray] and print it out.

    // ask user for date with GetValidDate(birthDateArray);
    // call CalculateDaysBetweenTwoDates(birthDateArray, currentDateArray) assign return value to daysBetweenTwoDates;
    // print out daysBetweenTwoDates
